...
def verify(origin_dir, image_width, image_height, image_suffix):
    """
    校验图片大小
    :return:
    """
    if not os.path.exists(origin_dir):
        print("【警告】找不到目录{}".format(origin_dir))
        exit()
    print("开始校验原始图片集")
    # 图片真实尺寸
...
def split(origin_dir, train_dir, test_dir, bad_imgs):
    ...
    if not os.path.exists(train_dir):
        print("找不到目录{}，将自动创建".format(train_dir))
        os.mkdir(train_dir)

    if not os.path.exists(test_dir):
        print("找不到目录{}，将自动创建".format(test_dir))
        os.mkdir(test_dir)
...
def main():
    with open("./conf/sample.json", "r") as f:
        sample_conf = json.load(f)
    # 图片路径
    image_dir = sample_conf["origin_image_dir"]
    ...
    bad_images_info = verify(image_dir, image_width, image_height, image_suffix)
    bad_imgs = []
